...
<body>
	...
	<div OnClick="location.href='/board/johabGroup?&sword=${page.getSword()}&belt=${page.getBelt()}&recurve=${page.getRecurve()}&spatula=${page.getSpatula() }&chain=${page.getChain()}&large=${page.getLarge()}&tear=${page.getTear()}&gloves=${page.getGloves()}&cloak=${page.getCloak()}'" style="width:750px" align="right" >
	조합 별 갯수 보기</div>
	...
	<div>	
	<c:if test="${page.prev}">
 		<span>[ <a href="/board/listPageSearch?num=${page.startPageNum - 1}${page.getSearchCondition()}">이전</a> ]</span>
	...
	<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
	 <span>
	  ...
	  <c:if test="${select != num}">
	   <a href="/board/listPageSearch?num=${num}${page.getSearchCondition()}">${num}</a>
	  </c:if>    
	...
	<c:if test="${page.next}">
	 <span>[ <a href="/board/listPageSearch?num=${page.endPageNum + 1}${page.getSearchCondition()}">다음</a> ]</span>
	</c:if>
	</div>
	...
	<script>
	 document.getElementById("searchBtn").onclick = function () {
	      ...
	      location.href = "/board/listPageSearch?num=1"+"&sword="+swordnum+"&belt="+ beltnum
